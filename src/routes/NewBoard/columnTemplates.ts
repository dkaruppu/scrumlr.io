...
export const columnTemplates: {[id: string]: {name: string; description?: string; columns: {name: string; hidden: boolean; color: Color}[]}} = {
  ...
  // },
  startStopContinue: {
    name: "Agile Retroboard",
    columns: [
      {name: "Kudos", hidden: false, color: "backlog-blue"},
      {name: "What Went Well", hidden: false, color: "grooming-green"},
      {name: "What Didn't Go Well", hidden: false, color: "online-orange"},
      {name: "Action Items", hidden: false, color: "planning-pink"},
    ],
  },
  // madSadGlad: {
  //   name: "Mad/Sad/Glad",
  ...
  swot: {
    ...
    columns: [
      ...
      {name: "Actions", hidden: false, color: "poker-purple"},
    ],
  },
};
